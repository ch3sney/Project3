...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    private static int mod(int a, int b) {
        ...
        // Declare modulo/remainder value
        int modulo = a % b;
        // If a is negative and remainder value is not 0
        if (a < 0 && modulo != 0) {
            // Modulo is equal to to b plus modulo
            modulo = b + modulo;
        }
        return modulo;
    }
    ...
    public final Pair<K, V> removeAny() {
        ...
        while (!found) {
            if (this.hashTable[bucket].size() > 0) {
                /*
                 * Check if size of current bucket is > 0 to satisfy requires
                 * clause of removeAny().
                 */
                this.size--;
                return this.hashTable[bucket].removeAny();
            } else {
                // Otherwise, increase bucket number
                bucket++;
            }
        }
    ...
    public final int size() {
        // Return size variable which is updated in other kernel methods
        return this.size;
    }
